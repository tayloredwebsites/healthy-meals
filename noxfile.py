...
def testing(session):
    ...
    try:
        session.run("uv", "run", "pytest", "tests",
                    ...
                    "--log-cli-level=WARN",  # do not output debug statements
...
def linting(session):
    ...
    session.run_install("uv", "sync", "--quiet", external=True)
    session.run("uv", "run", "pylint", "accounts", "pages", "common", "healthy_meals")
...
def dockerUpBg(session):
    ...
    logger.debug('*** nox -s dockerUpBg - started')
    session.run("docker", "compose", "up", "--build", "--detach")
    logger.debug('*** nox -s dockerUpBg - done')
...
def dockerEnsureUp(session):
    ...
    session.run_install("uv", "sync", "--quiet", external=True, )
    logger.debug('*** nox -s dockerEnsureUp - started')
    session.run("docker", "compose", "up", "--build", "--detach", "--no-recreate")
    logger.debug('*** nox -s dockerEnsureUp - done')
...
def dockerExecSh(session):
    ...
    session.run_install("uv", "sync", "--quiet", external=True, )
    logger.debug('*** nox -s dockerExecSh - started')
    session.run("docker", "exec", "-it", "healthy-meals-web-1", "sh")
    logger.debug('*** nox -s dockerExecSh - done')
...
def dockerDown(session):
    ...
    """
    session.run_install("uv", "sync", "--quiet", external=True, )
    logger.debug('*** nox -s dockerDown - started')
    ...
    logger.debug('*** nox -s dockerDown - done')
...
def dockerLogs(session):
    ...
    session.run_install("uv", "sync", "--quiet", external=True, )
    logger.debug('*** nox -s dockerLogs - started')
    session.run("docker", "compose", "logs", "--follow")
    logger.debug('*** nox -s dockerLogs - done')
...
def dockerTesting(session):
    ...
    """
    session.run_install("uv", "sync", "--quiet", external=True, )
    logger.debug('*** nox -s dockerTesting - started')
    session.run("docker", "exec", "-it", "healthy-meals-web-1", "nox", "-s", "testing")
    logger.debug('*** nox -s dockerTesting - done')
...
def setupEnv(session: object) -> None:
    ...
    # session.run_install("uv", "sync", "--quiet", external=True, )
    # # only run this when regenerating docs
...
def onlySphinxDocs(session: nox.Session):
    ...
    # Note: sphinx docs from manual .rst files from sphinx_src, and from code by apidocs
...
def cleanDocsBuild(session):
    ...
    session.run("uv", "run", "mkdir", "-p", "./docs/build/coverage_py/html/")
    session.run("uv", "run", "mkdir", "-p", "./docs/build/tests/")
    # # this is done only when being regenerated in "onlySphinxDocs"
...
def coverageReports(session):
    ...
    # generate the full coverage report (including .html and .txt files)
    ...
    # generate the Python only Coverage reporting
...
def dockerSphinxDocs(session):
    ...
    """
    # session.run_install("uv", "sync", "--quiet", external=True, )
    logger.debug('*** nox -s dockerSphinxDocs - started')
    session.run("docker", "exec", "-it", "healthy-meals-web-1", "nox", "-s", "sphinxDocs")
    logger.debug('*** nox -s dockerSphinxDocs - done')
