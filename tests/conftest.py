...
# @pytest.fixture
# def client_no_csrf():
#     return Client(enforce_csrf_checks=False)  # note: is default
...
def selenium(live_server, request):
    ...
    logger.debug(f'\n browser selection from request.param: {request.param}')
    ...
    match request.param:
        case "chromium":
            with open('/Users/dave/works/local_src/chrome_config.json') as f:
                ...
                logger.debug(f'*** all_configs_txt: {all_configs_txt}')
...
# ----------------------------------------------------------------------------------------------------------------------
# add @pytest.mark.slow options to tests
...
########################################################################################################################
#  Selenium testing utility functions
########################################################################################################################
...
def click_and_wait(driver: WebDriver, click_css: str, match_css: str):
    ...
    logger.debug(f'click_and_wait click_css: {click_css}')
    ...
    logger.debug(f'click_and_wait match_css: {match_css}')
...
def get_and_wait(driver: WebDriver, live_server: LiveServer, url: str):
    ...
    logger.debug(f'get_and_wait url: {live_server.url}{url}')
...
def screenshot(driver: WebDriver, filename_note: str = '', save_png: bool = False):
    ...
    logger.debug(f'screenshot started')
    logger.debug(f'screenshot filename_note: {filename_note}')
    ...
    logger.debug(f'screenshot 5')
    page_html = driver.page_source
    logger.debug(f'screenshot 6')
    with open(f'tmp/selenium/screen_{timestamp}_{note}.html', 'w') as f:
        f.write(page_html)
    logger.debug(f'screenshot finished')
