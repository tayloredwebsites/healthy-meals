...
tests/accounts/custom_user_cases.py file for configuration of CustomUser tests
...
class UserModelsTestCase(TransactionTestCase):
    ...
    def test_reset_sequences(self):
        ...
        logger.debug(f'*** {__name__}::test_reset_sequences - confirm get_case_superuser1() has run')
        assert hasattr(self, "user_info"), 'Missing the self.user_info from get_case_superuser1()'

        logger.debug(f'*** {__name__}::test_reset_sequences - Sign in self.signed_in_user')
        self.signed_in_user = self.user_info['user']
        self.client.login(email=self.user_info['email'], password=self.user_info['password'])

        logger.debug(f'*** {__name__}::test_reset_sequences - Get user from "get_case_superuser1()" fixture and "setUp()".')
        ...
        logger.debug(f'*** {__name__}::test_reset_sequences - confirm we have a new logged in superuser')
    ...
    def base_user_case(self):
        ...
        for rec in CustomUser.objects.all():
            logger.debug(f'*** {__name__}::test_custom_user - user: {rec}')
            # print(f'*** {__name__}::test_custom_user - user: {rec:detail}')

        logger.debug(f'*** {__name__}::test_case_custom_user - confirm we have a new logged in superuser')
        assert signed_in_user.id == 1, 'ID reset is not working, ID is not 1 (see notes)'
        assert signed_in_user.is_superuser, 'signed_in_user was not set up as superuser'
        assert signed_in_user.is_authenticated, 'signed_in_user is not logged in'

        logger.debug('*** {__name__}::test_case_custom_user - Initial Setup')
        ...
        logger.debug('*** {__name__}::test_case_custom_user - create 4 test users')
        ...
        logger.debug('*** {__name__}::test_case_custom_user - confirm we have 4 more users')
        ...
        logger.debug(f'*** {__name__}::test_case_custom_user - validate the created_at, updated_at, created_by, and updated_by fields')
        ...
        logger.debug(f'*** {__name__}::test_case_custom_user - validate user0 soft deletes & history (audit trail)')
        ...
        logger.debug(f'*** {__name__}::test_case_custom_user - Soft Delete of user0')
        user0.delete()
        ...
        logger.debug('*** %(name)s::test_case_custom_user - Soft Deleted of user: %(email)s: %(username)s, %(deleted)s', {'name': __name__, 'email': user0.email, 'username': user0.username, 'deleted': user0.deleted})
        ...
        logger.debug(f'*** {__name__}::test_case_custom_user - ensure the database does not allow duplicate users (with the same email), even if soft deleted')
        ...
        logger.debug(f'*** {__name__}::test_case_custom_user - validate deleted user is reflected properly in CustomUser methods')
        ...
        logger.debug(f'*** {__name__}::test_case_custom_user - undelete the soft deleted user')
        test_users[0].undelete()

        logger.debug(f'*** {__name__}::test_case_custom_user - confirm user undelete is reflected properly in CustomUser methods')
