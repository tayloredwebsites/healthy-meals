...
tests/accounts/atest_user_e2e.py
- Experimental End-to-end tests using selenium for user signup, login, and change password using the CustomUser model and allauth.
- Note: currently these test are not run, and the db and rest style tests are sufficient, efficient, and more reliable than selenium tests.  Selenium tests are slow and require a browser and a display, which is not always available in CI/CD pipelines.
...
def test_signup_e2e(selenium, live_server):
    ...
    logger.debug('live_server.url is %s', live_server.url)
    ...
    error_element = click_and_wait(driver=selenium, click_css='form#signup_form button',
                                   match_css='div#div_id_password1 .invalid-feedback')
    logger.debug('*** click_and_wait for bad password failure')
    assert selenium.title == 'Healthy Meals: Diet Assistant - Sign up'
    assert error_element.text == (
    ...
    logger.debug('*** body of the second message: %s', mail.outbox[email_2_ref].body)
...
def test_login_e2e(selenium, live_server):
    ...
    logger.debug(f'email_rec: {email_rec:detail}')
...
def test_change_pwd_e2e(selenium, live_server):
    ...
    # --------------------------------------------------------------------
    logger.debug('*** login with invalid password should fail')
    find_and_refill(selenium, find_css='input#id_login', fill=VALID_EMAIL)
    find_and_refill(selenium, find_css='input#id_password', fill=INVALID_PASSWORD)
    # time.sleep(5)
    screenshot(driver=selenium)
